...
class IOIEStudyAbroadProgram(Interface):
    ...
    rich_description = RichText(
        ...
        description=_(
            'This is the description that will be used to promote your program.  Your description should capture the purpose of your program, include an overview of what students will be engaged in while abroad/away, and capture students’ interest! (max length 600 chars)'),  # noqa
    ...
    learning_objectives = schema.List(
        ...
        description=_(
            'State the learning objectives for this program.  Include only one learning objective per text field. These learning objectives will be included in end-of-program assessment and may be used to support Higher Learning Commission and other accreditation processes. Press Tab to add more (max 12 entries)'),  # noqa
        required=True,
        value_type=DictRow(
    ...
    syllabus_and_supporting_docs = field.NamedFile(
        ...
        description=_(
            'Upload your syllabus; if you update your syllabus, replace this copy with the updated copy. This field will remain editable until just prior to travel. Additional documents can be added to this program folder once it has been saved.'),  # noqa
        required=False,
    )
    ...
    language_of_study = schema.List(
        ...
        description=_(
            'Select all that apply. Contact the Office of International Education to add a language (abroad@uwosh.edu).'),  # noqa
        required=False,
        value_type=schema.Choice(
    ...
    program_cycle = schema.Choice(
        ...
        description=_(
            'How often will this program be offered?  This information will display '
            'in some marketing materials. If it isn''t possible to predict, leave this blank.'
        ),
    ...
    individualInterview = schema.Choice(
        title=_('The Program Liaison, Program Leader or Program Co-leader will interview each applicant'),  # noqa
        vocabulary=yes_no_vocabulary,
        required=False,
        default='No',
    )
    ...
    firstRecommendationRequired = schema.Choice(
        ...
        description=_('If "yes", this item appears in the Applicant Portal as an application item'),  # noqa
        vocabulary=yes_no_vocabulary,
        required=False,
        default='No',
    )
    ...
    applicantQuestion1 = schema.Text(
        title=_('Question 1'),
        description=_(
            'You may add up to 5 short-answer questions to the participant application.  If a question appears here, it will be required of all applicants.  Questions cannot be made optional and cannot be applied to some applicants and not to others.'),  # noqa
        required=False,
        max_length=200,
    )
    ...
    applicantQuestion2 = schema.Text(
        title=_('Question 2'),
        description=_(
            'If a question appears here, it will be required of all applicants.  Questions cannot be made optional and cannot be applied to some applicants and not to others.'),  # noqa
        required=False,
        max_length=200,
    )
    ...
    cvRequired = schema.Choice(
        ...
        description=_(
            'If "yes", this item appears in the Applicant Portal as an application item.'),  # noqa
        vocabulary=yes_no_vocabulary,
        required=True,
        default='No',
    )
    ...
    program_coleaders = schema.List(
        ...
        description=_(
            'The On-site Program Co-Leader is responsible for providing leadership for the group and for overseeing group health and safety.  The On-site Program Co-Leader may also teach one or more of the program courses.'),  # noqa
        value_type=DictRow(
            title='co-leaders',
    ...
    courses = RichText(
        ...
        description='Include all courses that will be taught through this program.  Do not include courses that will be taught entirely at UWO, even when these courses are offered in preparation for the program away.  List existing courses only.  If the course you intend to use is not an existing course, your department must submit the course for formal approval through normal university channels prior to applying to use the course abroad/away.  Contact the OIE to add a course (abroad@uwosh.edu).',  # noqa
        required=False,
    ...
    program_schedule = schema.Choice(
        ...
        description=_('The OIE Program Manager checks this box to confirm that the program schedule has been viewed.  Checking this box does not mean that activities on the schedule have been approved.  Further Risk Assessment will be completed after this initial review.'),  # noqa
        vocabulary=yes_no_vocabulary,
        required=False,
    ...
    director_recommendations = RichText(
        ...
        description=_('Include site-specific Health, Safety & Security remarks when appropriate.'),  # noqa
        default_mime_type='text/plain',
        allowed_mime_types=('text/plain', 'text/html'),
        max_length=2500,
        required=False,
    ...
    health_safety_security_documents = RichText(
        ...
        description=_(
            'For all sites, upload the OIE Risk Assessment, Department of State Country Information, and CDC country-specific information.  When warranted, upload additional risk forms and/or supporting documentation.'),  # noqa
        required=False,
    ...
    add_health_document_link = RichText(
        required=False,
        default='<em>You can add health documents after saving this program</em>',  # noqa
    )
    ...
    application_items_label = schema.TextLine(
        ...
        description=_(
            'If checked "yes", the items below will appear in the Participant Portal as an application item.'),  # noqa
    ...
    flight_deviation_request_return_flight_only = schema.Choice(
        title=_('Application for Permission to Follow an Alternative Schedule on the Return Flight Only'),  # noqa
        description=_(
            ' Select ''yes'' for all independent travel programs where OIE does not hold space on a group flight and for all programs on which OIE holds space on a group flight but may allow deviations on the return flight only on a case-by-case basis.'),  # noqa
        vocabulary=yes_no_vocabulary,
        required=False,
    )
    flight_deviation_request_roundtrip_or_outbound_flight = schema.Choice(
        title=_('Application for Permission to Follow an Alternative Schedule on the Outbound Flight Only or on My Roundtrip Flights'),  # noqa
        description=_(
            'Select ''yes'' for all independent travel programs where OIE does not hold space on a group flight and for all programs on which OIE holds space on a group flight but may allow deviations on the outbound or roundtrip flights on a case-by-case basis.'),  # noqa
        vocabulary=yes_no_vocabulary,
        required=False,
    )
    hessen_isu_application = schema.Choice(
        ...
        description=_(
            'Select ''yes'' if this program is included in the Hessen/Wisconsin Student Exchange AND the program is an ISU (International Summer University).'),  # noqa
        vocabulary=yes_no_vocabulary,
        required=False,
    )
    hessen_iwu_application = schema.Choice(
        ...
        description=_(
            'Select ''yes'' if this program is included in the Hessen/Wisconsin Student Exchange AND the program is an IWU (International Winter University).'),  # noqa
        vocabulary=yes_no_vocabulary,
        required=False,
    )
    graduate_registration_form_and_graduate_special_non_degree_information_form = schema.Choice(  # noqa
        ...
        description=_(
            'Select ''yes'' 1) if the program allows undergraduate level course enrollment by graduate level students or 2) if the program offers graduate level courses and accepts applicants who will not be UW Oshkosh degree-seeking students during the term that the program runs.'),  # noqa
        vocabulary=yes_no_vocabulary,
        required=False,
    )
    transfer_credit_prior_approval_form = schema.Choice(
        ...
        description=_(
            'Select ''yes'' if one or more applicants may be enrolled in a host institution course that is eligible for transfer back to UW Oshkosh, without simultaneous enrollment at UW Oshkosh.'),  # noqa
        vocabulary=yes_no_vocabulary,
        required=False,
    )
    ...
    drivers_license_copy_for_india_visa_application = schema.Choice(
        ...
        description=_(
            'Select ''yes'' if required by external partners or by the foreign consulate to apply for a visa.'),  # noqa
        vocabulary=yes_no_vocabulary,
        required=False,
    )
    biographical_page_of_your_signed_passport = schema.Choice(
        title=_('Passport-Digital Copy of Biographical Page of Your SIGNED Passport'),  # noqa
        description=_(
    ...
    official_passport_photo_for_india_visa_application = schema.Choice(
        ...
        description=_(
            'Select ''yes'' if required by external partners or if required to apply for a visa with photo requirements that are consistent with an official U.S. passport photo.'),  # noqa
        vocabulary=yes_no_vocabulary,
        required=False,
    )
    ...
    passport_size_photo = schema.Choice(
        title=_('Photo: Paper'),
        description=_('Select ''yes'' if required by external partners or if required to apply for a visa with photo requirements that are not consistent with an official U.S. passport photo.'),  # noqa
        vocabulary=yes_no_vocabulary,
        required=False,
    )
    ...
    visa_required_for_us_citizens = schema.Choice(
        ...
        description=_("Select 'yes' if a visa is required of U.S. citizens."),  # noqa
        vocabulary=yes_no_vocabulary,
        required=False,
    )
    ...
    provider_proposals_label = schema.TextLine(
        description=_(
            '"Provider Proposals: At least 1 provider proposal must be selected and uploaded plus 1 flight proposal uploaded prior to using the ""Review Provider Proposal"" function. A yes/no contracting decision must be made for every provider and flight proposal prior to using the ""publish fee"" function.'),  # noqa
    )
    ...
    proposal_01 = field.NamedFile(
        title=_('Proposal 01'),
        description=_(
            'Upload a clean copy of proposal 01.  Proposal notes may be included by attaching these to the end of the clean proposal document.'),  # noqa
        required=False,
    )
    ...
    proposal_02 = field.NamedFile(
        title=_('Proposal 02'),
        description=_(
            'Upload a clean copy of proposal 03.  Proposal notes may be included by attaching these to the end of the clean proposal document.'),  # noqa
        required=False,
    )
    ...
    required_prior_to_publishing_initial_fee_label_2 = schema.TextLine(
        description=_(
            'Required Prior to Publishing Final Fee: Provider proposals and flight proposals on "Proposals" tab must also be complete.'),  # noqa
    )
    ...
    required_prior_to_confirming_ter_received_label = schema.TextLine(
        description=_('Required Prior to Confirming that the TER has been Received'),  # noqa
    )
    ...
    account_transfers = schema.Choice(
        ...
        description=_('Confirm that all transfers into and out of the account are complete.'),  # noqa
        vocabulary=yes_no_vocabulary,
        required=False,
    )
    program_revenue = schema.Choice(
        ...
        description=_(
            'Confirm that all program revenue has been received.  Confirm that the correct program revenue amount is in the account.'),  # noqa
        vocabulary=yes_no_vocabulary,
        required=False,
    )
    ...
    participant_orientation_url = schema.URI(
        ...
        description=_('Add detail on how to complete orientation.  Detail will show as participant instructions in the Participant Portal.'),  # noqa
        required=False,
    )

    mode(proof_of_service_label='display')
    proof_of_service_label = schema.TextLine(
        description=_('Required Prior to Scheduling the Operational Briefing'),  # noqa
    )
    ...
    first_day_of_spring_semester_classes = schema.Date(
        ...
        description='will be copied from the selected calendar year on first save',  # noqa
        required=False,
    )
